...
namespace KWeb.HttpOption
{
    public static class HttpRouteHandler
    {
        ...
        public static void RegisterPaths()
        {
            ...
            var types = allTypes.Where(t => t.IsSubclassOf(typeof(RestController)) && !t.IsAbstract);
            foreach (var type in types)
            {
                ...
                if(contruct!=null)
                {
                    ...
                    for (int i = 0; i < constructParams.Length; i++)
                    {
                        ...
                        {
                            object? value = ServiceProvider.Get(constructParams[i].ParameterType);
                            values[i] = value;
                        }
                    }
                }
                ...
                ServiceProvider.GlobalServices.AddService(type, () =>
                {
                    if (contruct != null)
                        return contruct.Invoke(values);
                    else
