...
namespace KWeb
{
    public class WebApplication : IDisposable
    {
                ...
                { 
                    ...
                    ThreadPool.QueueUserWorkItem(arg =>
                    {
                        ...
                        try
                        {
                            ...
                            stream.ReadTimeout = client.Available <= 2*QueueSize ? 10 : 50 * (int)(maxSize / bufferSize);
                            int recvBytes;
        ...
        private void RegisterInteceptors()
        {
            IEnumerable<Type> types = Assembly.GetEntryAssembly().GetTypes().Where(t => t.GetCustomAttribute<ServiceInjection>
            () != null && (t.IsSubclassOf(typeof(RequestInterceptor)) && !t.IsAbstract));
            foreach (Type type in types)
            {
                ServiceInjection injectionAttr = type.GetCustomAttribute<ServiceInjection>();
                interceptorManager.Interceptors.Add((RequestInterceptor)ServiceProvider.Get(type, injectionAttr.Name));
            }
        }
        public void Dispose()
        {
