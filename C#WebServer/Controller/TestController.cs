...
namespace Controller
{
    ...
    public class TestController : RestController
    {
        ...
        [ConfigInjection]
        private readonly ExampleConfig1 config1;
        [ServiceInjection]
        private readonly ExampleConfig2 config2;
        [ServiceInjection]
        ...
        public string Test9()
        {
            ...
            string? msg = form.GetValue("msg");
            logger.Debug(msg==null?"null":msg);
            return Test10();
        }
        ...
        [HttpGet]
        [Route("/Test19")]
        public async Task<ActionResult<string>> Test19()
        {
            return await Task.FromResult(OK<string>(Test10()));
        }

        [HttpGet]
        public ActionResult<string> Test20()
        {
            return OK<string>(File.ReadAllText("C#WebServer.deps.json"));
        }

        [HttpGet]
        public async Task<ActionResult<string>> Test21()
        {
            return await Task.Run(() => OK<string>(File.ReadAllText("C#WebServer.deps.json")));
        }

        [HttpGet]
        public ActionResult<string> Test22()
        {
            string a = $"config1: a:{config1.a},b:{config1.b}<br>";
            StringBuilder builder = new StringBuilder();
            builder.Append(a);
            builder.Append("config2:c:[");
            foreach(string s in config2.c)
            {
                builder.Append(s+',');
            }
            builder.Remove(builder.Length - 1, 1);
            builder.Append(']');
            builder.Append($",d:{config2.d}<br>");
        ...
        {
            try {
                redis.GetDatabase(2).Set("kk", config1.a, TimeSpan.FromMinutes(2));
                return OK();
            }
