    ...
    public abstract class KooberMySql<T, ID> : KooberBase<T, ID>
    {
        ...
        public override TR? SelectOne<TR>(string command, Func<T, TR> result, params object[] values) where TR : default
        {
            ...
            if(result == null)
            {
                return ObjectUtil.CopyProperties(res,
                    Activator.CreateInstance<TR>(),BindingFlags.Public| BindingFlags.Instance);
            }
            return result(res);
        }
        ...
        public override List<T> Query(PagedQuery<T> page, string command, params object[] values)
        {
            ...
            using (DbCommand cmd = connection.CreateCommand())
            {
                ...
                cmd.CommandText = $"select count(*) from {tableName} where {command.Substring(command.IndexOf("where")+7)}";
                using (DbDataReader reader = cmd.ExecuteReader())
                {
                    if(reader.Read())
        ...
        public override List<TR> Query<TR>(PagedQuery<TR> page, string command, params object[] values)
        {
            PagedQuery<T> query = PagedQuery<T>.Create(page.Page, page.Size);
            Query(query, command, values);
            foreach (T item in query.Data)
            {
                page.Data.Add(ObjectUtil.CopyProperties(item,Activator.CreateInstance<TR>(),
                    BindingFlags.Instance|BindingFlags.Public));
            }
            return page.Data;
        }

        public override List<TR> Query<TR>(PagedQuery<TR> page, string command, Func<T, TR> result, params object[] values)
        {
            PagedQuery<T> query = PagedQuery<T>.Create(page.Page, page.Size);
            Query(query, command, values);
            return query.Data.Select(result).ToList();
        }
        ...
        public override List<TR> Query<Q, TR>(PagedQuery<TR> page, string command, Q query)
        {
            return Query(page,command, ObjectToArray(query));
        }
        public override List<TR> Query<Q, TR>(PagedQuery<TR> page, string command, Func<T, TR> result, Q query)
        {
            return Query(page,command,result,ObjectToArray(query));
        }
    }
